...
--------------------------------------------------------------------------------------------------------------------------------
 *@ 1.0.0.1         26-Oct-2020                  Mansur Farooq              Bug Fix: Owner of the image can edit/delete the image.
*/
...
public class ImageRepository {
    ...
    public Image getImageByTitle(String title,int id) {
        EntityManager em = emf.createEntityManager();
        TypedQuery<Image> typedQuery = null; //Added by Mansur Farooq. Fix for image upload issue.
        try {
            //typedQuery = (TypedQuery<Image>) em.createQuery("SELECT i from Image i where i.title =:title , Image.class)setParameter("title",title);;//Comments added by Mansur Farooq. Fix for image upload issue.
            typedQuery = (TypedQuery<Image>) em.createQuery("SELECT i from Image i where i.title =:title and i.id =:id", Image.class); //Added by Mansur Farooq. Fix for image upload issue.
            //Start: Added by Mansur Farooq. Fix for image upload issue.
            typedQuery.setParameter("title",title);
            typedQuery.setParameter("id",id);
            //End: Added by Mansur Farooq. Fix for image upload issue.
